...
def get_dim_company_from_stock(dim_stock_path='/opt/airflow/data/dim_stock.csv', save_path='/opt/airflow/data/dim_company.csv', batch_size=50, **kwargs):
    ...
    dim_company['charter_capital'] = dim_company['charter_capital'].apply(
        lambda x: '{:,}'.format(x) if pd.notna(x) and x is not None else ''
    )
    dim_company.to_csv(save_path, index=False, encoding='utf-8-sig', lineterminator='\n', quoting=1)
...
with DAG(
    ...
    description='Weekly update dim_company data from vnstock',
    ...
    run_dim_company_update = PythonOperator(
        ...
        op_kwargs={
            ...
            'save_path': '/opt/airflow/data/dim_company.csv',
            'batch_size': 50
        }
    )
