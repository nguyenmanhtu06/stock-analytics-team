...
def update_stock_price_nearest_to_postgres(symbol, table_name, engine):
    ...
    try:
        ...
        raise RuntimeError(f"Lỗi khi cập nhật {symbol}: {e}")
...
def run_update_all_symbols():
    ...
    try:
        df_symbols = pd.read_sql(
            f"SELECT DISTINCT symbol FROM {table_name} ORDER BY symbol ASC LIMIT 10", engine
        )
...
with DAG(
    dag_id="el_hourly_update",
    default_args=default_args,
    description="Extract & Load dữ liệu mỗi giờ",
    schedule_interval="@hourly",  # chạy mỗi 1 giờ
